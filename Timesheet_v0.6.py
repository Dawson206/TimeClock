...
class TimePunchApp(ctk.CTk):
    def __init__(self):
        ...
        # Override the window close event to add exit confirmation
    ...
    def on_close(self):
        ...
        if messagebox.askyesno("Exit", "Are you sure you want to exit?"):
            # Log only if clocked in and a file is selected
            if self.clock_in_button.cget("state") == "disabled" and self.filename:
                timestamp = datetime.now().strftime("%m-%d-%Y %H:%M:%S")
                entry = f"[{timestamp}] App closed while clocked in\n\n"
                with open(self.filename, "a") as f:
                    f.write(entry)
            self.destroy()
